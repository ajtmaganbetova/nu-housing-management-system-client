      ...
      >
        <div className="flex items-center gap-4">
          ...
          <div>
            <p className="text-sm font-semibold text-gray-900">
              {info["Name Surname"] || `Student #${application.student_id}`}
              {(info["School"] || application.major) ? ` — ${info["School"] || application.major}` : ""}
            </p>
            <p className="text-xs text-gray-500">
              {info["Student ID"] ? `NU ID: ${info["Student ID"]} · ` : `Student DB ID: ${application.student_id} · `}
        ...
        <div className="flex items-center gap-3">
          {application.status === "pending" && (
            <>
              <button
                onClick={(e) => { e.stopPropagation(); onApprove(application.id); }}
        ...
        <div className="border-t border-gray-200 px-6 py-5 space-y-6 bg-gray-50">
          ...
          {application.status === "approved" && (
            <div className="rounded-md bg-green-50 border border-green-200 p-3">
              <p className="text-sm font-medium text-green-800">✓ Application has been approved.</p>
            </div>
          )}
          ...
          <div>
            ...
            <div className="space-y-2">
              ...
              <InfoRow label="Student ID" value={info["Student ID"] || String(application.student_id)} />
...
export default function HousingApplicationsTable({ onStatsUpdate }: { onStatsUpdate?: () => void }) {
    ...
    <div>
      ...
      {filteredApplications.length === 0 ? (
        <div className="text-center py-16 bg-white rounded-lg border border-gray-200">
          ...
          <p className="text-gray-400 text-sm">No applications match the selected filter.</p>
        </div>
      ) : (
